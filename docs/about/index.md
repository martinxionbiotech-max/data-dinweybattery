---
description: About the DINWEY Battery Knowledge Hub — who publishes this documentation, our editorial standards, and how to contact us.
type: page
date_published: 2026-08-30
date_modified: 2026-08-30
---

# About This Knowledge Hub

The **DINWEY Battery Knowledge Hub** is the official technical documentation
sub-site of [dinweybattery.com](https://dinweybattery.com).

## Who We Are

DINWEY is the truck and heavy-duty starting-battery brand of **Chengguang Energy**,
a battery manufacturer founded in 2002.

| Fact | Detail |
|---|---|
| Brand | DINWEY |
| Parent company | Chengguang Energy (founded 2002) |
| Facility | 200,000 m², Jinzhou City, Shijiazhuang, Hebei, China |
| Production | 18 automated lines, 40,000 batteries/day |
| Focus | Truck / heavy-duty / commercial-vehicle starting batteries (SLI) |
| Certifications | IATF 16949, ISO 9001:2015, ISO 14001:2015, ISO 45001:2018, OHSAS 18001, CE |
| Contact | martin@dinweys.com · WhatsApp +86 13323237275 |

## What This Hub Is For

This hub provides technical documentation to help engineers, fleet managers,
distributors and buyers specify the right truck battery:

- Battery standards (JIS, DIN/EN, BCI, SAE J537)
- Cold Cranking Amps (CCA) and Reserve Capacity (RC) explained
- Selection guides and comparison tables
- Maintenance and troubleshooting
- Application scenarios (long-haul, cold-chain, mining, semi-trailer, generator)

## Our Editorial Standards

1. **Sourced specifications** — all CCA, capacity and dimension values come from
   Chengguang Energy technical documentation under an IATF 16949 quality system.
2. **No unverifiable claims** — where a value depends on the vehicle, we say so and
   direct you to your manual.
3. **Team authorship** — content is written and reviewed by the DINWEY Technical Team.
   We do not publish attributed articles under invented individual experts.
4. **Transparent dates** — every article carries publication and update dates.

## How to Use This Documentation

- **Buyers & distributors** — start with the [Selection Guide](../selection-guide/index.md)
  and [Buying Guide](../buying-guide/index.md).
- **Fleet managers** — see [Fleet Battery Management](../fleet-battery-management/index.md).
- **Engineers** — see [Battery Standards](../jis-vs-din-vs-bci/index.md) and
  [12V vs 24V](../12v-vs-24v/index.md).

## Contact

For product datasheets, OEM/private-label programs and quotations, visit the
[main site contact page](https://dinweybattery.com/contact/) or email
martin@dinweys.com.
